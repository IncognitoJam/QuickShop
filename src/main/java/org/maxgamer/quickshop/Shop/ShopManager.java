            ...
            /* Purchase Handling */
                ...
                if (shop.isSelling()) {
                    ...
                    if (!p.getUniqueId().equals(shop.getOwner())) {
                        ...
                        double taxRate = taxExemption ? 0 : plugin.getConfig().getDouble("tax");

                        // Calculate the amount of tax this purchase would require
                        double taxForPurchase = amount * shop.getPrice() * taxRate;

                        // Ensure the player has enough money to pay for the product + tax
                        if (plugin.getEcon().getBalance(p.getName()) < (amount * shop.getPrice()) + taxForPurchase) {
                            p.sendMessage(MsgUtil.getMessage("you-cant-afford-to-buy", format(amount * shop.getPrice()), format(taxForPurchase), format(plugin.getEcon().getBalance(p.getName()))));
                            return;
                        }
                        ...
                        if (!shop.isUnlimited() || plugin.getConfig().getBoolean("shop.pay-unlimited-shop-owners")) {
                            plugin.getEcon().deposit(shop.getOwner(), total * (1 - taxRate));
                            if (taxRate != 0) {
                                plugin.getEcon().deposit(plugin.getConfig().getString("tax-account"), total * taxRate);
                            }
                        }
                        // Notify the shop owner
                        if (plugin.getConfig().getBoolean("show-tax")) {
                            String msg1 = MsgUtil.getMessage("player-bought-from-your-store-tax", p.getName(), "" + amount, shop.getDataName(), Util.format((taxRate * total)));
                            if (stock == amount)
                            ...
                            String msg1 = MsgUtil.getMessage("player-bought-from-your-store", p.getName(), "" + amount, shop.getDataName());
                            if (stock == amount)
                                msg1 += "\n" + MsgUtil.getMessage("shop-out-of-stock", "" + shop.getLocation().getBlockX(), "" + shop.getLocation().getBlockY(), "" + shop.getLocation().getBlockZ(), shop.getDataName());
                            MsgUtil.send(shop.getOwner(), msg1);
                        }
                    ...
                    if (!p.getUniqueId().equals(shop.getOwner())) {
                        ...
                        String msg1 = MsgUtil.getMessage("player-sold-to-your-store", p.getName(), "" + amount, shop.getDataName());
                        if (space == amount)
