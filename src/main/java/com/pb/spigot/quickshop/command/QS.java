package com.pb.spigot.quickshop.command;

import com.pb.spigot.quickshop.database.Database;
import com.pb.spigot.quickshop.shop.*;
import org.bukkit.*;
...
import com.pb.spigot.quickshop.database.MySQLCore;
import com.pb.spigot.quickshop.database.SQLiteCore;
...
public class QS implements CommandExecutor {
    ...
    private void create(CommandSender sender) {
        if (sender instanceof Player) {
            ...
            if (item.getType() != Material.AIR && (!plugin.sneakCreate || p.isSneaking())) {
